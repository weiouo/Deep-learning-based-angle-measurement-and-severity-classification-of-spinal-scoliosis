...
def run_batch_evaluation(opt):
    ...
    for filename in os.listdir(image_dir):
        ...
        # 只使用 y_pred 的範圍來評估（保持原設定）
        y_eval = np.linspace(min(y_pred), max(y_pred), 300)

        # poly fit 預測
        poly = np.poly1d(np.polyfit(y_pred, x_pred, 3))
        x_poly = poly(y_eval)
        ...
        # 中心化後計算斜率差異（重視曲線形狀）
        x_poly_centered = x_poly - np.mean(x_poly)
        x_spline_centered = x_spline - np.mean(x_spline)

        mse = mean_squared_error(x_spline_centered, x_poly_centered)
        r2 = r2_score(x_spline_centered, x_poly_centered)
        mse_list.append(mse)
        r2_list.append(r2)
        ...
        plt.plot(x_poly, y_eval, 'g-', label='Poly Fit')
