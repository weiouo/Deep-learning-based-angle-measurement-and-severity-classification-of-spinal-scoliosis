...
def run_batch_evaluation(opt):
    ...
    for filename in os.listdir(image_dir):
        ...
        k = min(3, len(x_pred) - 1)
        if k < 1:
            continue

        spline_fit = UnivariateSpline(y_pred, x_pred, k=k, s=0)
        x_spline_fit = spline_fit(y_eval)

        spline = CubicSpline(y_gt, x_gt)
        x_spline = spline(y_eval)

        x_spline_fit_centered = x_spline_fit - np.mean(x_spline_fit)
        x_spline_centered = x_spline - np.mean(x_spline)

        mse = mean_squared_error(x_spline_centered, x_spline_fit_centered)
        r2 = r2_score(x_spline_centered, x_spline_fit_centered)
        mse_list.append(mse)
        r2_list.append(r2)
        ...
        plt.plot(x_spline, y_eval, 'r-', label='Ground Truth (Spline)')
        plt.plot(x_spline_fit, y_eval, 'g-', label='Spline Fit')
    ...
    plt.xlabel("Number of Detected Points")
    plt.ylabel("MSE")
    plt.title("MSE vs. Number of Detected Points")
    plt.grid(True)
    plt.savefig("result/mse_vs_point_count.png", bbox_inches='tight')
    plt.close()
