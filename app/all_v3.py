...
def run_combined_visualization(opt):
    ...
    for filename in os.listdir(image_dir):
        ...
        try:
            fits['Cubic'] = CubicSpline(y_pred, x_pred)(y_eval)
        except:
            continue
        try:
            poly_fit = Polynomial.fit(y_pred, x_pred, 3)
            fits['Poly'] = poly_fit(y_eval)
        except:
            continue
        try:
            lowess_fit = lowess(x_pred, y_pred, frac=0.3, return_sorted=True)
            fits['LOESS'] = np.interp(y_eval, lowess_fit[:, 0], lowess_fit[:, 1])
        except:
            continue
        try:
            spline_fit = UnivariateSpline(y_pred, x_pred, k=min(3, len(x_pred)-1), s=0)
            fits['Spline'] = spline_fit(y_eval)
        except:
            continue

        # Ground Truth
        gt_spline = CubicSpline(y_gt, x_gt)
        x_gt_spline = gt_spline(y_eval)

        # 繪圖
        plt.figure(figsize=(6, 6))
        plt.imshow(orig_img, cmap='gray')
        plt.plot(x_gt_spline, y_eval, '-', label='GT Spline', color='red', linewidth=3)
        color_map = {
            'Cubic': 'lime',
            'Poly': 'deepskyblue',
            'LOESS': 'magenta',
        ...
        selected_methods = ['LOESS', 'Spline']  # 可在這裡自訂要顯示的兩個方法
        for method in selected_methods:
            if method in fits:
                x_fit = fits[method]
                plt.plot(x_fit, y_eval, '-', label=method, color=color_map.get(method, 'black'), linewidth=2.5)
        plt.legend()
        plt.axis('off')
        plt.savefig(os.path.join("result", f"{os.path.splitext(filename)[0]}_combined_curve.png"), bbox_inches='tight', pad_inches=0)
        plt.close()
...
if __name__ == "__main__":
    ...
    run_combined_visualization(opt)
